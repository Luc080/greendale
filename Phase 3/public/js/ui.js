...
function renderBuildTab() {
  ...
  html += '</div>';
  if (state.buildMode)
    html += '<p style="margin-top:5px;font-size:11px;color:var(--green-dark);font-weight:700">📍 Klick auf Karte — <span style="cursor:pointer;text-decoration:underline" onclick="cancelBuild()">abbrechen</span></p>';
  document.getElementById('tab-content').innerHTML = html;
}
...
function renderChainsTab() {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    html += '<div class="chain-row"><span>'+nm+'</span><span style="color:var(--text-light)">→</span>';
    if (c.input) {
...
function renderSidebarOrders() {
  ...
  for (var i = 0; i < state.orders.length; i++) {
    ...
    html += '<div class="order-card'+(ok?' completable':'')+'"><div class="order-label">'+o.label+'</div>';
    html += '<div class="order-footer"><span class="order-reward">+'+o.xp+' XP</span>';
    html += '<button class="order-btn"'+(ok?'':' disabled')+' onclick="fulfillOrder(\''+o.id+'\')">'+( ok?'✓ Liefern':'warten')+'</button></div></div>';
  }
...
function renderSidebarVillagers() {
  ...
  for (var i = 0; i < state.villagers.length; i++) {
    ...
    html += '<div class="vil-mini"><span style="font-size:15px">'+v.emoji+'</span><span class="v-name">'+v.name+'</span><span class="v-status '+sc+'">'+st2+'</span><div class="vil-hunger">'+dots+'</div></div>';
  }
  // Nächster hire
  for (var i = 0; i < VILLAGER_POOL.length; i++) {
    var vp = VILLAGER_POOL[i], hired = false;
    for (var j = 0; j < state.villagers.length; j++) {
      if (state.villagers[j].name === vp.name) { hired = true; break; }
    }
    if (!hired) {
      var can = state.xp >= vp.reqXP;
      html += '<button class="hire-btn"'+(can?'':' disabled')+' onclick="hireVillager('+i+')">';
      html += vp.emoji+' '+vp.name+' einstellen'+(can?'':' (⭐'+vp.reqXP+')');
      html += '</button>';
      break;
    }
  }
...
function renderSidebarPlayers() {
  ...
  for (var i = 0; i < onlinePlayers.length; i++) {
    ...
    html += '<div class="player-card"><div class="p-dot"></div><span class="p-name">'+p.name+'</span><span class="p-xp">⭐'+p.xp+'</span></div>';
  }
  if (onlinePlayers.length === 0) html = '<p style="font-size:11px;color:var(--text-light)">Keine Mitspieler online</p>';
  el.innerHTML = html;
}

function renderSidebarResources() {
  var items = [
    {i:'🪵',k:'wood',     n:'Holz'   },{i:'🪨',k:'stone',    n:'Stein' },
    {i:'🌾',k:'wheat',    n:'Weizen' },{i:'🍲',k:'soup',     n:'Suppe' },
    {i:'🪑',k:'furniture',n:'Moebel' },{i:'🧱',k:'brick',    n:'Ziegel'},
    {i:'🥖',k:'bread',    n:'Brot'   },{i:'💧',k:'water',    n:'Wasser'}
  ];
  var html = '';
...
function showBuildingPopup(b, px, py) {
  ...
  document.getElementById('popup-title').textContent = (bt?bt.emoji+' '+bt.name:b.type);
