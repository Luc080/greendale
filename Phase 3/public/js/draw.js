...
// ============================================================
// KAMERA: Startposition zentriert auf Kartenmitte
// ============================================================
function resizeCanvas() {
  ...
  // Karte in der Bildschirmmitte zentrieren
  // Kartenmittelpunkt in Iso: col=COLS/2, row=ROWS/2
  var midC = COLS / 2, midR = ROWS / 2;
  oX = canvas.width  / 2 - (midC - midR) * (TW / 2);
  oY = canvas.height / 2 - (midC + midR) * (TH / 2);
}
...
// ============================================================
// KARTE GENERIEREN – grösser dank COLS/ROWS aus data.js
// ============================================================
...
function drawTile(c, r) {
  ...
  if (type === 3) {
    ...
    ctx.beginPath();
    ctx.moveTo(p.x - 9, p.y + TH/2 - 1);
    ctx.lineTo(p.x + 9, p.y + TH/2 - 1);
...
function drawBuilding(b) {
  ...
  var W = 58, H = 48, bx = x - W/2, by = y - H + TH/2;
  var sel = (state.selectedBuilding === b.id);

  ctx.save(); ctx.globalAlpha = .2;
  ctx.beginPath(); ctx.ellipse(x, y+TH/2+2, 32, 12, 0, 0, Math.PI*2);
  ctx.fillStyle = '#000'; ctx.fill(); ctx.restore();

  if (sel) {
    ctx.beginPath(); ctx.ellipse(x, y+TH/2+2, 38, 15, 0, 0, Math.PI*2);
  ...
  ctx.strokeStyle = sel ? '#f0a500' : 'rgba(0,0,0,0.22)';
  ctx.lineWidth   = sel ? 2.5 : 1.5; ctx.stroke();

  ctx.beginPath(); ctx.roundRect(bx-2, by-13, W+4, 20, 8);
  ctx.fillStyle = st.roof; ctx.fill();
  ctx.strokeStyle = 'rgba(0,0,0,0.18)'; ctx.lineWidth = 1.2; ctx.stroke();
  ctx.beginPath(); ctx.roundRect(x, by-13, (W/2)+2, 20,
    { upperLeft:0, upperRight:8, lowerRight:8, lowerLeft:0 });
  ctx.fillStyle = 'rgba(0,0,0,0.08)'; ctx.fill();

  // Fenster
  ctx.beginPath(); ctx.roundRect(bx+8, by+H-26, 12, 12, 4);
  ctx.fillStyle = 'rgba(255,252,180,0.88)'; ctx.fill();
  ctx.strokeStyle = 'rgba(0,0,0,0.18)'; ctx.lineWidth = 1; ctx.stroke();
  ctx.beginPath(); ctx.roundRect(bx+W-20, by+H-26, 12, 12, 4);
  ctx.fillStyle = 'rgba(255,252,180,0.88)'; ctx.fill(); ctx.stroke();
  ctx.strokeStyle = 'rgba(0,0,0,0.15)'; ctx.lineWidth = 0.8;
  ctx.beginPath(); ctx.moveTo(bx+14,  by+H-26); ctx.lineTo(bx+14,  by+H-14); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(bx+8,   by+H-20); ctx.lineTo(bx+20,  by+H-20); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(bx+W-14,by+H-26); ctx.lineTo(bx+W-14,by+H-14); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(bx+W-20,by+H-20); ctx.lineTo(bx+W-8, by+H-20); ctx.stroke();

  // Tür
  ctx.beginPath(); ctx.roundRect(x-7, by+H-20, 14, 20,
    { upperLeft:7, upperRight:7, lowerLeft:0, lowerRight:0 });
  ctx.fillStyle   = shade(st.wall, -35); ctx.fill();
  ctx.strokeStyle = 'rgba(0,0,0,0.2)'; ctx.lineWidth = 1; ctx.stroke();
  ctx.beginPath(); ctx.arc(x+5, by+H-10, 2, 0, Math.PI*2);
  ctx.fillStyle = '#f0c040'; ctx.fill();

  // Gebäude-spezifische Extras
  if (b.type === 'townhall') {
    ctx.beginPath(); ctx.roundRect(x-7, by-27, 14, 16, 4);
    ctx.fillStyle = st.accent; ctx.fill();
    ctx.strokeStyle = 'rgba(0,0,0,0.18)'; ctx.lineWidth = 1; ctx.stroke();
    ctx.beginPath(); ctx.moveTo(x-10, by-27); ctx.lineTo(x, by-42); ctx.lineTo(x+10, by-27); ctx.closePath();
    ctx.fillStyle = st.roof; ctx.fill(); ctx.stroke();
    ctx.strokeStyle = '#888'; ctx.lineWidth = 1.2;
    ctx.beginPath(); ctx.moveTo(x, by-42); ctx.lineTo(x, by-33); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(x, by-42); ctx.lineTo(x+9, by-38); ctx.lineTo(x, by-34); ctx.closePath();
    ctx.fillStyle = '#e05252'; ctx.fill();
  } else if (b.type === 'kitchen' || b.type === 'bakery') {
    var cx2 = b.type === 'kitchen' ? x+14 : x-14;
    ctx.beginPath(); ctx.roundRect(cx2-4, by-28, 9, 20, 2);
    ctx.fillStyle = shade(st.wall, -20); ctx.fill();
    ctx.strokeStyle = 'rgba(0,0,0,0.15)'; ctx.lineWidth = 1; ctx.stroke();
    ctx.save(); ctx.globalAlpha = .28;
    ctx.beginPath(); ctx.arc(cx2, by-32, 6, 0, Math.PI*2); ctx.fillStyle = '#ccc'; ctx.fill();
    ctx.beginPath(); ctx.arc(cx2-2, by-38, 4, 0, Math.PI*2); ctx.fill();
    ctx.restore();
  } else if (b.type === 'sawmill') {
    ctx.save(); ctx.translate(bx+W-11, by+7);
    ctx.beginPath(); ctx.arc(0, 0, 8, 0, Math.PI*2);
    ctx.fillStyle = '#d0d0d0'; ctx.fill(); ctx.strokeStyle = '#999'; ctx.lineWidth = .8; ctx.stroke();
    for (var i = 0; i < 6; i++) {
      ctx.save(); ctx.rotate(i * Math.PI / 3);
      ctx.beginPath(); ctx.moveTo(6,0); ctx.lineTo(9,2); ctx.lineTo(9,-2); ctx.closePath();
      ctx.fillStyle = '#aaa'; ctx.fill(); ctx.restore();
    }
    ctx.restore();
  } else if (b.type === 'farm') {
    ctx.font = '13px serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'bottom';
    ctx.fillText('🌾', bx+10, by+H+2); ctx.fillText('🌾', bx+W-10, by+H+2);
  } else if (b.type === 'well') {
    ctx.beginPath(); ctx.arc(x, by-2, 20, Math.PI, 0);
    ...
    ctx.beginPath(); ctx.moveTo(x, by-2); ctx.lineTo(x, by+9); ctx.stroke();
    ctx.beginPath(); ctx.arc(x, by+H/2-4, 5, 0, Math.PI*2);
