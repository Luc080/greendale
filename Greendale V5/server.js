...
// ============================================================
// AUFTRAGS-TEMPLATES (Phase 4: mehr Vielfalt + Coins)
// ============================================================
...
function generateOrders(count = 4) {
  const orders = [];
...
function startRandomEvent(valley) {
  ...
  console.log(`Valley ${valley.id}: Event gestartet – ${t.title}`);

  // Auto-Ende nach duration
  setTimeout(() => {
...
function getEventSummary(ev) {
  if (!ev) return null;
  return {
    id: ev.id, title: ev.title, desc: ev.desc,
    goal: ev.goal, progress: ev.progress,
    endsAt: ev.endsAt, completed: ev.completed,
    reward: ev.reward
  };
}
...
function getOrCreateValley(valleyId) {
  if (!valleys[valleyId]) {
    valleys[valleyId] = {
      ...
      activeEvent: null,
      eventTimer: null,
      coins: {} // { playerId: amount } – serverseitig getrackt
    };
    // Nach 5 Minuten erstes Event starten
    setTimeout(() => {
...
wss.on('connection', (ws) => {
    ...
    switch (msg.type) {
      ...
      case 'join': {
        ...
        valley.players[playerId] = {
          ...
          state: msg.state || null, joinedAt: Date.now()
        };

        // DB-Stand laden (wenn verfügbar)
        let dbState = null;
        if (supabase) {
          dbState = await dbLoadPlayer(playerName);
        }
        ...
        broadcastToValley(valleyId, {
          ...
          playerName, players: getPlayerList(valley)
        }, playerId);
      ...
      case 'state_update': {
        ...
        // DB speichern (gedrosselt – nur wenn explizit gewünscht)
      ...
      case 'fulfill_order': {
        ...
        // Neuen Auftrag
        const t = ORDER_TEMPLATES[Math.floor(Math.random() * ORDER_TEMPLATES.length)];
        valley.orders.push({ id: uuidv4(), items: { ...t.items }, xp: t.xp, coins: t.coins || 5, label: t.label });

        const playerName = valley.players[playerId]?.name || 'Jemand';
        const coinsEarned = order.coins || 5;

        // Event-Fortschritt prüfen
        if (valley.activeEvent && !valley.activeEvent.completed) {
          const ev = valley.activeEvent;
          for (const res in ev.goal) {
            if (order.items[res]) {
              ev.progress[res] = (ev.progress[res] || 0) + (order.items[res] || 0);
              if (ev.progress[res] > ev.goal[res]) ev.progress[res] = ev.goal[res];
            }
          }
          // Prüfen ob Event abgeschlossen
          ...
          if (done) {
            ...
            broadcastToValley(currentValleyId, {
              ...
              reward: ev.reward,
              message: `🎉 ${ev.title} abgeschlossen! +${ev.reward.xp} XP +${ev.reward.coins} Coins für alle!`
            }, null);
            valley.activeEvent = null;
            // Nächstes Event in 10 Minuten
            setTimeout(() => {
              if (valleys[currentValleyId]) startRandomEvent(valleys[currentValleyId]);
            }, 10 * 60 * 1000);
          } else {
            broadcastToValley(currentValleyId, {
              type: 'event_progress', event: getEventSummary(ev)
            }, null);
          }
        }

        broadcastToValley(currentValleyId, {
          type: 'orders_updated', orders: valley.orders,
          message: `${playerName} hat geliefert: ${order.label} (+${order.xp} XP, +${coinsEarned} 🪙)`
        }, null);
        sendToPlayer(ws, {
          type: 'orders_updated', orders: valley.orders,
          xpGained: order.xp, coinsGained: coinsEarned,
          message: `Du hast geliefert: ${order.label} (+${order.xp} XP, +${coinsEarned} 🪙)`
        });
        break;
      }
      ...
      case 'trade_offer': {
        if (!currentValleyId || !valleys[currentValleyId]) return;
        const valley = valleys[currentValleyId];
        const targetPlayer = Object.values(valley.players).find(p => p.name === msg.targetName);
        if (!targetPlayer) {
          sendToPlayer(ws, { type: 'error', message: `Spieler "${msg.targetName}" nicht gefunden` });
          return;
        }
        const offererName = valley.players[playerId]?.name || 'Jemand';
        sendToPlayer(targetPlayer.ws, {
          type: 'trade_incoming',
          fromId: playerId, fromName: offererName,
          offer: msg.offer,    // { wood: 5, ... }
          request: msg.request // { stone: 3, ... }
        });
        break;
      }
      ...
      case 'trade_accept': {
        ...
        sendToPlayer(partner.ws, {
          ...
          receive: msg.request,
          give: msg.offer,
          partnerName: valley.players[playerId]?.name
        });
        break;
      }
      ...
      case 'chat': {
        ...
        broadcastToValley(currentValleyId, {
          type: 'chat', playerId, playerName, text: chatMsg, timestamp: Date.now()
        }, null);
        break;
      }
  ...
  ws.on('close', () => {
    ...
    broadcastToValley(currentValleyId, {
      type: 'player_left', playerId, playerName, players: getPlayerList(valley)
    }, null);
...
function getPlayerList(valley) {
  return Object.values(valley.players).map(p => ({
    id: p.id, name: p.name, joinedAt: p.joinedAt, xp: p.state?.xp || 0
  }));
}

server.listen(PORT, () => {
  console.log(`\n🌿 Greendale Server Phase 4 läuft!`);
