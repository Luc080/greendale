...
var DAY_PHASES = [
  ...
  { name:'🌙 Nacht',  skyTop:'#1a1a3a', skyBot:'#0a0820',
    sunColor:'#4455aa', sunIntens:0.2, ambIntens:0.15, sx:0,  sy:2,   sz:-1 }
];
...
function _makeStdBuilding3D(g,W,H,D,ms,type) {
  ...
  // Dach (Pyramide, 4 Seiten = 45° gedreht)
  var roofH=H*.55;
  var roof=new THREE.Mesh(new THREE.ConeGeometry(W*.72,roofH,4),ms.roof.clone());
  roof.rotation.y=Math.PI/4; roof.position.set(0,H+.09+roofH/2,0); roof.castShadow=true; g.add(roof);
  // Fenster vorne
  var wM=M.window.clone();
  var wG=new THREE.BoxGeometry(W*.18,H*.22,.04);
  var w1=new THREE.Mesh(wG,wM); w1.position.set(-W*.22,H*.56+.09,D/2+.02); g.add(w1);
  var w2=new THREE.Mesh(wG,wM.clone()); w2.position.set(W*.22,H*.56+.09,D/2+.02); g.add(w2);
  // Fenster Seite
  var wGS=new THREE.BoxGeometry(.04,H*.22,D*.18);
  var w3=new THREE.Mesh(wGS,wM.clone()); w3.position.set(W/2+.02,H*.56+.09,0); g.add(w3);
  // Tür
...
function _addDetails3D(g,type,W,H,D) {
  // Schornstein
  if (type==='kitchen'||type==='bakery'||type==='smithy') {
    var ch=new THREE.Mesh(new THREE.BoxGeometry(W*.12,H*.5,W*.12),M.chimney.clone());
    ch.position.set(W*.28,H+.09,D*.15*-1); ch.castShadow=true; g.add(ch);
    var sm=new THREE.Mesh(new THREE.SphereGeometry(.08,6,4),mLamb('#999999'));
    sm.material.transparent=true; sm.material.opacity=0.38;
    sm.position.set(W*.28,H*1.3+.09,-D*.15); sm.userData.isSmoke=true; g.add(sm);
  }
  // Sägeblatt
  if (type==='sawmill') {
...
function animateVillagers(dt) {
  for (var i=0;i<state.villagers.length;i++) {
    ...
    v.anim=(v.anim||0)+(spd>.005?.055:.008);
...
function syncBuildings() {
  ...
  for(var bid in buildingMeshes){
    ...
    bg.traverse(function(o){
      if(o.isMesh&&o.material&&!o.userData.isHitBox)
        o.material.emissive=isSel?new THREE.Color(0x443300):new THREE.Color(0x000000);
    });
  }
}
...
function updateDayLight() {
  ...
  if(renderer) renderer.setClearColor(lerpHexColor(dp.cur.skyTop,dp.next.skyTop,dp.t));
