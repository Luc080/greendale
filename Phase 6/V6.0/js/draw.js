...
var ctx       = null;
var oX = 0, oY = 0;
var camZoom   = 1.0;   // Phase 6: Zoom
var CAM_ZOOM_MIN = 0.35;
var CAM_ZOOM_MAX = 2.2;
var waterAnim = 0;
var camDrag   = false;
...
// ── Tages-/Nacht-Zyklus ──────────────────────────────────────
var dayNightTick = 0;
var DAY_CYCLE_LENGTH = 3600; // Phase 6: halber Zyklus = deutlicher sichtbar
...
function getNightOverlayAlpha() {
  var f = getDayNightFactor();
  // Phase 6: stärkeres Overlay für besseren Kontrast
  if (f >= 0.65) return 0;
...
function initCameraDrag() {
  ...
  canvas.addEventListener('wheel', function(e) {
    ...
    var newZoom = Math.max(CAM_ZOOM_MIN, Math.min(CAM_ZOOM_MAX, camZoom * delta));

    // Kamera so verschieben dass der Maus-Punkt stabil bleibt
    oX = mouseX - (mouseX - oX) * (newZoom / camZoom);
    oY = mouseY - (mouseY - oY) * (newZoom / camZoom);
    camZoom = newZoom;
  }, { passive: false });
  ...
  canvas.addEventListener('touchmove', function(e) {
    if (e.touches.length === 2) {
      ...
      if (_pinchDist > 0) {
        var delta2  = dist / _pinchDist;
        var newZoom2 = Math.max(CAM_ZOOM_MIN, Math.min(CAM_ZOOM_MAX, camZoom * delta2));
        ...
        oX = cx - (cx - oX) * (newZoom2 / camZoom);
        oY = cy - (cy - oY) * (newZoom2 / camZoom);
        camZoom = newZoom2;
      }
      _pinchDist = dist;
    }
...
function drawBuilding(b) {
  ...
  // Level-2: Gebäude 15% grösser; alles mit camZoom skalieren
  var lvlScale = (b.level >= 2) ? 1.15 : 1.0;
  var W = Math.round(sz.W * lvlScale * camZoom), H = Math.round(sz.H * lvlScale * camZoom);
  var bx = x-W/2, by = y-H+TH/2*camZoom;

  ctx.save(); ctx.globalAlpha = .2;
  ctx.beginPath(); ctx.ellipse(x, y+TH/2+2, W*.55*camZoom, 12*camZoom, 0, 0, Math.PI*2);
  ctx.fillStyle = '#000'; ctx.fill(); ctx.restore();

  if (sel) {
    ctx.beginPath(); ctx.ellipse(x, y+TH/2+2, W*.62*camZoom, 14*camZoom, 0, 0, Math.PI*2);
  ...
  // Level-2: Goldener Rahmen + Badge
  if (b.level >= 2) {
    // Goldener Aussenrahmen
    ctx.save();
    ...
    // Badge
    ctx.save();
    ctx.beginPath(); ctx.arc(bx+W, by+2, 10, 0, Math.PI*2);
    var badgeGrad = ctx.createRadialGradient(bx+W, by+2, 2, bx+W, by+2, 10);
    badgeGrad.addColorStop(0, '#ffe066');
    badgeGrad.addColorStop(1, '#f0a500');
    ctx.fillStyle = badgeGrad; ctx.fill();
    ctx.strokeStyle = '#fff'; ctx.lineWidth = 1.5; ctx.stroke();
    ctx.font = 'bold 8px Nunito,sans-serif';
    ctx.fillStyle = '#5a2a00'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText('L2', bx+W, by+2);
    ctx.restore();
  }
}
...
function _drawBuildingLabel(b, bt, x, by, H, W) {
  var labelSize = Math.max(7, Math.round(10 * camZoom));
  ctx.font = 'bold ' + labelSize + 'px Nunito,sans-serif'; ctx.fillStyle = b.type==='casino'?'#ffd700':'#3a2a1a';
...
function drawVillager(v) {
  var p = toIso(v.x, v.y), px = p.x, py = p.y+TH/2*camZoom-4*camZoom;
  ...
  var nameSize = Math.max(6, Math.round(8 * camZoom));
  ctx.font = 'bold ' + nameSize + 'px Nunito,sans-serif'; ctx.fillStyle = '#2a1a0a';
  ctx.textAlign = 'center'; ctx.textBaseline = 'top';
...
function draw() {
  ...
  // Zoom-Indikator (verschwindet nach 2s)
  if (Math.abs(camZoom - 1.0) > 0.05) {
    ctx.save();
    ctx.globalAlpha = 0.7;
