// ============================================================
// GREENDALE SERVER – v6.5
// Express + WebSockets + Supabase-Persistenz
// ============================================================
...
// ============================================================
// SUPABASE
// Env-Variablen haben Vorrang, sonst Fallback auf hardcoded
// ============================================================
const SUPABASE_URL = process.env.SUPABASE_URL || 'https://kwtbtxebobjysxyxtsmg.supabase.co';
const SUPABASE_KEY = process.env.SUPABASE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imt3dGJ0eGVib2JqeXN4eXh0c21nIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4MDAwNjk0MCwiZXhwIjoyMDk1NTgyOTQwfQ.Vcgc3FJz51V0QnJ2nG5CF-JRujn4y9OxHGC9jwaklQU';
...
async function dbLoadPlayer(playerName) {
  ...
  try {
    ...
    console.warn('dbLoad error:', e.message);
    return null;
  }
}
...
async function dbSavePlayer(playerName, stateData) {
  ...
  try {
    ...
    if (error) console.warn('dbSave error:', error.message);
  } catch(e) {
    console.warn('dbSave exception:', e.message);
  }
}
  ...
  ws.on('message', async (raw) => {
    ...
    switch (msg.type) {
      ...
      case 'join': {
        ...
        let dbState = null;
        if (supabase) {
          const row = await dbLoadPlayer(playerName);
          if (row) { dbState = row.state; console.log(`DB-Stand geladen: ${playerName}`); }
        }
        ...
        broadcastToValley(valleyId, {
          type: 'player_joined', playerId, playerName, players: getPlayerList(valley)
        }, playerId);
      ...
      case 'state_update': {
        ...
        if (msg.persist && supabase && msg.state) {
          await dbSavePlayer(msg.state.playerName, msg.state);
        }
      ...
      case 'fulfill_order': {
        ...
        if (valley.activeEvent && !valley.activeEvent.completed) {
          ...
          for (const res in ev.goal) {
            if (order.items[res]) {
              ev.progress[res] = Math.min(ev.goal[res], (ev.progress[res] || 0) + (order.items[res] || 0));
            }
          }
          ...
          if (done) {
            ev.completed = true;
            broadcastToValley(currentValleyId, {
              type: 'event_completed', reward: ev.reward,
      ...
      case 'trade_offer': {
        ...
        sendToPlayer(target.ws, {
          type: 'trade_incoming', fromId: playerId,
          fromName: valley.players[playerId]?.name || 'Jemand',
          offer: msg.offer, request: msg.request
        });
        break;
      }
...
server.listen(PORT, () => {
  console.log(`\n🌿 Greendale Server v6.5 läuft!`);
  console.log(`   Lokal:   http://localhost:${PORT}`);
  console.log(`   Valley:  http://localhost:${PORT}?valley=meintal`);
  console.log(`   DB:      ${supabase ? '✅ Supabase aktiv' : '⚠️  kein Supabase'}\n`);
});
