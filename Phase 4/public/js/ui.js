// ============================================================
// UI.JS – Phase 4
// Neu: Casino-Tab, Events, Handel-Popup, Coins-Animation
// ============================================================
...
function switchTab(tab) {
  ...
  var tabs = ['villagers', 'build', 'chains', 'casino', 'trade', 'chat'];
...
// CASINO TAB
// ============================================================
var bjState = null; // Blackjack-Zustand
...
function renderBlackjackActive() {
  ...
  // Zeige nur erste Karte des Dealers
...
function startBlackjack(bet) {
  ...
  bjState.dealerValue = bjHandValue([bjState.dealer[0]]); // nur erste Karte
  // Prüfen auf Blackjack direkt
  if (bjHandValue(bjState.player) === 21) {
    bjStand();
    return;
  }
  renderActiveTab();
...
function bjHit() {
  ...
  var val = bjHandValue(bjState.player);
  if (val > 21) {
    bjFinish('bust');
  } else {
    renderActiveTab();
...
// Event-Timer im Hintergrund aktualisieren
setInterval(function() {
  if (state.activeEvent) updateEventDisplay();
}, 10000);

// ============================================================
// BUILDING POPUP (FIX: name/emoji aus BUILDING_TYPES)
// ============================================================
function showBuildingPopup(b, px, py) {
  var pop = document.getElementById('info-popup');
  // FIX PHASE 3→4: name/emoji kommen aus BUILDING_TYPES, nicht aus b
