// ============================================================
// STATE.JS – Phase 4
// Neu: Coins, Schmiede (2 Inputs), Supabase-Persist
// ============================================================
...
var state = {
  ...
  nextVillagerId:   3,
  prevXP:           0,
  activeEvent:      null,  // NEU Phase 4
  tradeOffer:       null,  // NEU Phase 4
  villagers: [
...
function saveGame() {
  try {
    ...
    // Auch ans Server-DB senden (Supabase)
...
function loadGame(playerName) {
  try {
    // Version 7 zuerst probieren, dann v6 als Fallback
...
// DB-Stand vom Server laden und mergen
function applyDbState(dbState) {
  if (!dbState) return;
  // Nur überschreiben wenn DB-Stand neuer ist (mehr XP oder mehr Tag)
...
function moveVillagers() {
  for (var i = 0; i < state.villagers.length; i++) {
    ...
    var vm = Math.sqrt(v.vx*v.vx + v.vy*v.vy), maxSpd = 0.035;
