// ============================================================
// UI.JS – Phase 5
// Neu: Flüssiger Festival-Timer, Leaderboard, Settings, Upgrade
// ============================================================
...
// Level-Up Effekt (animierter)
function showLevelUpEffect() {
  var el = document.getElementById('levelup-overlay');
  if (!el) return;
  var lvl = getLevel(state.xp);
  var box = el.querySelector('.levelup-box');
  if (box) box.textContent = '🏅 LEVEL ' + lvl + '! 🎉';
...
function switchTab(tab) {
  ...
  var tabs = ['villagers', 'build', 'chains', 'casino', 'trade', 'chat', 'leaderboard'];
...
function renderActiveTab() {
  ...
  else if (state.activeTab === 'leaderboard') renderLeaderboardTab();
}
...
function renderVillagerTab() {
  ...
  for (var i = 0; i < state.villagers.length; i++) {
    ...
    var tt  = v.buildingId !== null ? t('idle').replace('😴 ','🔨 ') + ' ' + v.task : t('idle');
    if (v.buildingId !== null) {
      var bname = '';
      for (var j = 0; j < state.buildings.length; j++)
        if (state.buildings[j].id === v.buildingId) { bname = BUILDING_TYPES[state.buildings[j].type].name; break; }
      tt = '🔨 ' + bname;
    }
  ...
  if (!html) html = '<p style="font-size:12px;color:var(--text-light);padding:4px">' + t('noVillagers') + '</p>';
...
function renderBuildTab() {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    html += '<div class="build-cost">🪵'+bt.costWood+' 🪨'+bt.costStone+extra+'</div></div></div>';
  }
  html += '</div>';
  if (state.buildMode)
    html += '<p style="margin-top:6px;font-size:12px;color:var(--green-dark);font-weight:700">' +
            t('buildHint') + '<span style="cursor:pointer;text-decoration:underline" onclick="cancelBuild()">' + t('cancelBuild') + '</span></p>';
  document.getElementById('tab-content').innerHTML = html;
}
function startBuild(tp) { state.buildMode=(state.buildMode===tp)?null:tp; renderBuildTab(); }
...
function renderChainsTab() {
  ...
  // Nur Gebäude anzeigen die gebaut sind
  var builtTypes = {};
  for (var i = 0; i < state.buildings.length; i++) builtTypes[state.buildings[i].type] = true;
  for (var i = 0; i < keys.length; i++) {
    var k = keys[i], c = CHAINS[k];
    if (!c.output) continue;
    if (!builtTypes[k]) continue;
    ...
    if (c.inputB) {
      var ok2 = (state.resources[c.inputB]||0) >= c.inputAmtB;
      html += '<span class="chain-step '+(ok2?'ready':'waiting')+'">'+c.inputAmtB+'x '+c.inputB+'</span>';
      html += '<span style="color:var(--text-light)">→</span>';
    }
    html += '<span class="chain-step ready">+'+c.outputAmt+' '+c.output+'</span></div>';
  }
  if (!html) html = '<p style="font-size:12px;color:var(--text-light)">Noch keine Produktionsgebäude gebaut.</p>';
  document.getElementById('tab-content').innerHTML = html;
}
...
function renderCasinoTab() {
  ...
  if (lvl < 5) {
    document.getElementById('tab-content').innerHTML =
      ...
      t('casinoLocked') + lvl + t('casinoXPMissing') + (950 - state.xp) + t('casinoXPSuffix') +
      '</div>';
    return;
  }
  if (!hasCasino) {
    document.getElementById('tab-content').innerHTML =
      '<div style="padding:10px;text-align:center;color:var(--text-light);font-size:12px">' +
      t('casinoBuildFirst') +
      '<button class="tab-action-btn" onclick="startBuild(\'casino\');switchTab(\'build\')">' + t('casinoBuildNow') + '</button>' +
      '</div>';
    return;
  }
...
function renderBlackjackActive() {
  ...
  // Dealer: bei laufendem Spiel eine Karte verdeckt, bei finished alle zeigen
  html += '<div class="bj-hand"><span class="bj-label">Dealer:</span> ';
  if (bj.finished) {
    for (var i = 0; i < bj.dealer.length; i++) html += renderCard(bj.dealer[i]);
    html += ' <span class="bj-score">(' + bjHandValue(bj.dealer) + ')</span>';
  } else {
    html += renderCard(bj.dealer[0]) + ' <span class="bj-hidden">🂠</span>';
    html += ' <span class="bj-score">(' + bj.dealerValue + '?)</span>';
  }
  html += '</div>';
  ...
  html += '<div class="bj-result" id="bj-result">' + (bj.resultMsg || '') + '</div>';
...
function playSlot(bet) {
  if ((state.coins||0) < bet) { showNotif(t('notEnoughCoins')); return; }
  state.coins -= bet;
  ...
  for (var i = 0; i < 3; i++) {
    (function(idx, result) {
      var frames = 0, maxFrames = 12 + idx * 4;
...
function renderLeaderboardTab() {
  ...
  html += '<div style="font-size:12px;font-weight:900;color:var(--text);margin-bottom:6px">' + t('leaderboardTitle') + '</div>';
  if (onlinePlayers.length === 0) {
    html += '<p style="font-size:12px;color:var(--text-light)">' + t('leaderboardEmpty') + '</p>';
  } else {
    var sorted = onlinePlayers.slice().sort(function(a,b) { return (b.xp||0) - (a.xp||0); });
    var medals = ['🥇','🥈','🥉'];
    for (var i = 0; i < sorted.length; i++) {
      var p   = sorted[i];
      var xp  = typeof p.xp === 'number' ? p.xp : 0;
      var lvl = getLevel(xp);
      var isMe = (p.id === myPlayerId);
      var medal = medals[i] || (i+1 + '.');
      html += '<div class="leaderboard-row' + (isMe?' me':'') + '">';
      html += '<span class="lb-rank">' + medal + '</span>';
      html += '<span class="lb-name">' + p.name + (isMe ? t('leaderboardYou') : '') + '</span>';
      html += '<span class="lb-lvl">Lvl ' + lvl + '</span>';
      html += '<span class="lb-xp">⭐' + xp + '</span>';
      html += '</div>';
    }
  }
  html += '</div>';
...
function renderTradeTab() {
  ...
  if (onlinePlayers.length <= 1) {
    html += '<p style="font-size:12px;color:var(--text-light)">' + t('noTradePartners') + '</p>';
  } else {
    html += '<div style="font-size:11px;font-weight:700;color:var(--text);margin-bottom:5px">' + t('tradeTitle') + '</div>';
    html += '<div style="font-size:11px;color:var(--text-light);margin-bottom:6px">' + t('tradeChoose') + '</div>';
    ...
    html += tradeResInput(t('tradeOffer'), 'trade-offer');
    html += tradeResInput(t('tradeRequest'), 'trade-request');
    html += '</div>';
    html += '<button class="order-btn" style="width:100%;padding:5px" onclick="sendTradeOffer()">' + t('tradeSend') + '</button>';
  }
  html += '</div>';
  document.getElementById('tab-content').innerHTML = html;
}
...
function sendTradeOffer() {
  ...
  if (Object.keys(offer).length === 0 && Object.keys(request).length === 0) {
    showNotif(t('tradeEmpty')); return;
  }
  networkSendTradeOffer(target.value, offer, request);
  showNotif(t('tradeSent') + target.value + t('tradeSentSuffix'));
}
...
function showTradePopup(msg) {
  ...
  html += '<div style="font-weight:800;margin-bottom:5px">' + t('tradeFrom') + '<strong>'+msg.fromName+'</strong></div>';
  html += '<div style="font-size:12px;margin-bottom:3px">' + t('tradeOffers') + '<strong>'+offerStr+'</strong></div>';
  html += '<div style="font-size:12px;margin-bottom:8px">' + t('tradeWants') + '<strong>'+reqStr+'</strong></div>';
  html += '<button class="order-btn" style="margin-right:5px" onclick="acceptTrade(\''+msg.fromId+'\')">' + t('tradeAccept') + '</button>';
  html += '<button class="order-btn" style="background:#e05252" onclick="hideTradePopup()">' + t('tradeDecline') + '</button>';
  html += '</div>';
...
function acceptTrade(partnerId) {
  var tr = state.tradeOffer;
  if (!tr) return;
  networkAcceptTrade(partnerId, tr.offer, tr.request);
  hideTradePopup();
}
...
function renderChatTab() {
  ...
  html += '</div>';
  html += '<div class="chat-input-row"><input id="chat-input" type="text" placeholder="' + t('chatPlaceholder') + '" maxlength="100" onkeydown="chatKeydown(event)"><button onclick="sendChat()">➤</button></div>';
  document.getElementById('tab-content').innerHTML = html;
...
function renderSettingsContent() {
  ...
  html += '<div style="font-size:12px;font-weight:800;margin-bottom:4px">' + t('settingsLang') + '</div>';
  html += '<div style="display:flex;gap:6px">';
  html += '<button class="settings-lang-btn' + (LANG==='de'?' active':'') + '" onclick="setLang(\'de\')">🇩🇪 Deutsch</button>';
  html += '<button class="settings-lang-btn' + (LANG==='en'?' active':'') + '" onclick="setLang(\'en\')">🇬🇧 English</button>';
  html += '</div></div>';
  html += '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">';
  html += '<span style="font-size:12px;font-weight:800">' + t('settingsSounds') + '</span>';
  html += '<button class="settings-toggle" onclick="toggleSounds()">' + (window.soundsEnabled ? t('settingsOn') : t('settingsOff')) + '</button>';
  html += '</div>';
  html += '<button class="settings-close-btn" onclick="hideSettings()">' + t('settingsClose') + '</button>';
  sc.innerHTML = html;
}
...
function updateTabLabels() {
  var tabKeys = ['villagers','build','chains','casino','trade','chat','leaderboard'];
  var labelKeys = ['tabVillagers','tabBuild','tabChains','tabCasino','tabTrade','tabChat','tabLeaderboard'];
  var btns = document.querySelectorAll('.tab-btn');
  for (var i = 0; i < btns.length && i < labelKeys.length; i++) {
    btns[i].textContent = t(labelKeys[i]);
    btns[i].setAttribute('onclick', "switchTab('" + tabKeys[i] + "')");
  }
}
...
function renderSidebar() {
  ...
  // Sidebar-Titel auf aktuelle Sprache setzen
  var titles = document.querySelectorAll('.sidebar-title');
  var keys = ['sidebarValley','sidebarOrders','sidebarVillagers','sidebarPlayers','sidebarResources'];
  // (statisch in HTML definiert – nur bei Sprachwechsel nötig)
}
...
function renderSidebarOrders() {
  ...
  for (var i = 0; i < state.orders.length; i++) {
    ...
    html += '<button class="order-btn"'+(ok?'':' disabled')+' onclick="fulfillOrder(\''+o.id+'\')">'+(ok?t('deliver'):t('wait'))+'</button>';
    html += '</div></div>';
  }
  if (!html) html = '<p style="font-size:12px;color:var(--text-light)">' + t('waitOrders') + '</p>';
...
function renderSidebarVillagers() {
  ...
  for (var i = 0; i < state.villagers.length; i++) {
    ...
    var st2 = v.buildingId !== null ? t('working') : t('idle');
...
function renderSidebarPlayers() {
  ...
  if (onlinePlayers.length === 0)
    html = '<p style="font-size:12px;color:var(--text-light)">' + t('noPlayers') + '</p>';
  el.innerHTML = html;
}
...
// ============================================================
// VALLEY EVENT DISPLAY – Phase 5: flüssiger Timer mit Date.now()
// ============================================================
function updateEventDisplay() {
  ...
  var now  = Date.now();
  var secs = Math.max(0, Math.floor((ev.endsAt - now) / 1000));
...
function showBuildingPopup(b, px, py) {
  ...
  if (ch && ch.output) {
    ...
    var outAmt = ch.outputAmt * (b.level >= 2 ? 2 : 1);
    descParts.push(outAmt + 'x ' + ch.output + (b.level >= 2 ? ' (⬆️2x)' : ''));
  }
  ...
  if (b.type === 'casino') {
    ...
    btn.textContent = t('openCasino');
    btn.onclick = function() { hidePopup(); switchTab('casino'); };
    ac.appendChild(btn);
  } else if (ch && ch.output) {
    // Upgrade-Button (nur für upgradeable Gebäude, Level 1)
    if (b.level < 2 && UPGRADE_COSTS[b.type]) {
      var uc = UPGRADE_COSTS[b.type];
      var upBtn = document.createElement('button');
      upBtn.className = 'assign-btn';
      upBtn.style.background = '#f0a500';
      upBtn.style.borderColor = '#c88000';
      upBtn.textContent = t('upgradeBtn') + '2' + t('upgradeSuffix') + ' 🪵' + uc.wood + ' 🪨' + uc.stone;
      upBtn.style.pointerEvents = 'all';
      upBtn.onclick = (function(bid) { return function() { upgradeBuilding(bid); }; })(b.id);
    ...
    for (var i = 0; i < idle.length; i++) {
      (function(v, bld, chain) {
        var btn = document.createElement('button');
        btn.className = 'assign-btn';
        btn.textContent = v.emoji + ' ' + v.name + t('assignBtn');
    ...
    for (var i = 0; i < wrks.length; i++) {
      (function(v) {
        ...
        btn.textContent = v.emoji + ' ' + v.name + t('removeBtn');
        btn.style.pointerEvents = 'all';
        btn.onclick = function() {
          v.buildingId = null; v.task = 'Idle'; v.progress = 0;
          hidePopup(); renderSidebar(); renderActiveTab();
          showNotif(v.name + ' ' + t('idle'));
        };
        ac.appendChild(btn);
      })(wrks[i]);
    }
